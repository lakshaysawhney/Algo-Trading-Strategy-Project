...
def generate_target_position_threshold(context, data):
    ...
    for security in context.securities:
        if context.signals[security] > 0:
            # Example: Larger positions for stocks near lower Bollinger Band
            lower_band_threshold = 0.3
            weight = 0.1 if context.dist_to_lower_band[security] < lower_band_threshold else 0.05
            context.target_position[security] = weight * context.params['leverage']
        elif context.signals[security] < 0:
            # Example: Larger short positions for stocks near upper Bollinger Band
            upper_band_threshold = 0.7
            weight = 0.1 if context.dist_to_upper_band[security] > upper_band_threshold else 0.05
            context.target_position[security] = -weight * context.params['leverage']
        else:
            context.target_position[security] = 0
